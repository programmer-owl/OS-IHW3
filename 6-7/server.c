...
int n = 0;
int bearSock, beeSock, monitorSock; /* Socket descriptors for bear and beehive */
int pid = 0;
...
void sigint_handler(int signum) {
  if (pid == 0) {
    my_unlink();
    char* mes = "You've stopped the program. The bees are grateful for saving them "
         "from the bear!\n";
      send(monitorSock, mes, RCVBUFSIZE, 0);
  }
...
void HandleBearClient() {
  ...
  while (1) {
    ...
    send(monitorSock, buffer, RCVBUFSIZE, 0);
  }
...
void HandleBeehiveClient() {
  ...
  while (1) {
    ...
    if (recvMsgSize == 0) 
      break;
    char mes[RCVBUFSIZE];
    sprintf(mes, "Received info from the hive (number of bees and number of portions of honey): %s", echoBuffer);
    send(monitorSock, mes, RCVBUFSIZE, 0);
...
int main(int argc, char *argv[]) {
  ...
  monitorSock = AcceptTCPConnection(servSock);
